...
#Both Valuse set to false on program start, which is triggered is used for the pot file and hash upload functionality
...
#Pot File - Create a New Potfile when using a Single wordlist
def pot_file():
    global pot_file
    global hash_path_and_name
    pot_name = raw_input("Enter Name for pot file, or Press 1 for the same name as filename previously selected" +'\n')
...
#Exactly 6 character words - Oxford Dictionary Starting with UPPER Case - {upto 3 ANY Characters LEFTSIDE, upto 3 ANY Characters RIGHT SIDE}
def hc_command_menu_8():
    #Three ANY CHARACTERS (Left Side)
    subprocess.call(app + attack_mode_inc_left + hash_type_NetNTLMv2 + hash_path_and_name + pot_file + three_any_characters + wordlist_directory + cmd_defaults + increment, shell=True)
    #Three ANY CHARACTERS (Right Side)
    subprocess.call(app + attack_mode_inc_right + hash_type_NetNTLMv2 + hash_path_and_name + pot_file + wordlist_directory + three_any_characters + cmd_defaults + increment, shell=True)
...
#Crack Menu 8 - Exactly 6 character words - Oxford Dictionary Starting with UPPER Case - {upto 3 ANY Characters LEFTSIDE, upto 3 ANY Characters RIGHT SIDE})
def crack_menu_8():
    global wordlist_directory
    wordlist_directory = "/opt/wordlists/english-words/words-6-only_first_letter_upper.txt"
    pot_file()
    hashcat_command_line_menu()
    hc_command_menu_8()
...
def crack_menu():
    ...
    try:
        while 1:
            ...
            prCyan("8) Automated Testing - Exactly 6 character words - Oxford Dictionary Starting with UPPER Case - {upto 3 ANY Characters LEFTSIDE, upto 3 ANY Characters RIGHT SIDE})")
