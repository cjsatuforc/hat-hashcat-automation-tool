...
def crack_menu():
    ...
    try:
        while 1:
            ...
            print("5) Automated Testing - Try Oxford Dictionary(min >=8chars) + Starting with UPPER Case + upto 3 ANY Characters on RIGHT SIDE - Use for Corporate Clients - (Runtime ~4min)")
            print("6) Automated Testing - Try Oxford Dictionary(min >=8chars) + Starting with UPPER Case + upto 3 ANY Characters on LEFT SIDE - Use for Corporate Clients - (Runtime ~4min)")
...
def hash_from_file():
    ...
    for list_dir in os.listdir('../hash_upload/'):
            print list_dir
            hash_upload = raw_input("Select the file from the above list to be uploaded: ")
            os.chdir(hash_upload_dir)
#           if ( not os.path.isfile(hash_upload)):
#                print("Error: %s file not found" % hash_upload)
#            else:
#                print("Hash File %s found and accepted..." % hash_upload)
            if (os.path.isfile(hash_upload)):
                print("Hash File %s found and accepted..." % hash_upload)
                hash_abs_path == (os.path.join(hash_upload_dir, hash_upload))
